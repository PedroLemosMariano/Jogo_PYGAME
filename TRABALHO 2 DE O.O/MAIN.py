...
class Jogo:
    def __init__(self):
        ...
        self.telafinal.read()
        self.hiscore = self.telafinal.scores[0]
        self.segundo = self.telafinal.scores[1]
        self.terceiro = self.telafinal.scores[2]
    ...
    def rodar(self):
        while True:
            ...
            if not self.gameover:
                ...
                self.telafinal.read()
                self.telafinal.update(self.pontuação.pontos, self.player_name)
                self.telafinal.write()
                self.telafinal.draw(self.tela)
                self.pontuação.pontos = 0
                self.canal1.stop()
                self.tela.blit(self.pontuação.text_surface, (750, 590))
